...
public class Main extends PApplet{
	...
	public void setup(){
		...
		point_n.setFill(color(0,0,0));
		point = createShape(ELLIPSE, 0, 0, 15, 15);
		point0 = createShape(ELLIPSE, 0, 0, 5, 5);
		point1 = createShape(ELLIPSE, 0, 0, 15, 15);
		point1.setFill(color(204, 102, 0));
		point2 = createShape(ELLIPSE, 0, 0, 20, 20);
		point2.setFill(color(15,232,59));
		...
		gg = new GraphGeneration(6, 16, "Square");
		...
		if(command == 0){
			...
			for (Vertex v : gg.getList()) {
				if(set_backbone.contains(v)){
					...
					textAlign(CENTER);
					text(v.ID, (float)v.x*680, ((float)v.y*680 + 3));
				}else if(set_neighbor.contains(v))
					shape(point_n, (float)v.x*680, (float)v.y*680);
				else 
